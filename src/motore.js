...
export function calcolaNetto(input, parametri = PARAMETRI_DEFAULT) {
  ...
  // Gli oneri deducibili escono dalla busta paga come i contributi: il fondo
  // pensione lo trattiene il datore. Vanno quindi sottratti anche dal netto, non
  // solo dall'imponibile — altrimenti la cascata mostrerebbe una sottrazione che
  // il totale non fa. Il vantaggio non e' l'importo dedotto ma il risparmio
  // d'imposta che genera.
  const nettoAnnuo = euro(ral - totaleTrattenute - oneriDeducibili + totaleBonus);
...
function calcolaNettoSemplice(input, parametri) {
  ...
  return ral - contributi.totale - oneri - irpefNetta - addizionali.totale + sommaEsente + ti;
}
